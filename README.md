...
**カード決済、QRコード決済に続く「第三の決済手段」。** ExPress は、オンライン口座の残高で支払い、カード番号を店舗に渡さずに買い物を完結させる決済サービスです。買う人は支払い手段をひとつにまとめられ、売る人はSDKを数行組み込むだけで、承認・売上確定・返金・入金までを任せられます。このリポジトリは、その製品像を決済基盤の内側から自作したポートフォリオ実装です。表示名は **ExPress**、コード識別子は `express-wallet` / `@exw/*`。

利用者・加盟店2社・運営者を訪問者ごとの隔離workspaceへ生成し、残高払い、模擬カード直接払い、部分売上確定、返金、出金、継続課金を、実DB・複式台帳・独立ワーカー経由で処理します。サンプルEC「NORTHSTAR」は別オリジン・別DB・別権限で動作し、サーバーSDK・公開API・署名付きWebhookだけで ExPress と連携します。店舗側にカード番号や利用者の認証情報は一切渡りません。
...
- **通貨**：日本円（JPY）単体。金額はすべて整数（最小通貨単位）で保持し、丸め誤差が出ない設計です。
