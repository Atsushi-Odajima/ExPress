...
const client=(s:any)=>new ExPressClient({baseUrl:config.apiUrl,...JSON.parse(decrypt(s.credentials))});
...
await app.listen({host:'127.0.0.1',port:config.storePort});console.log(`NORTHSTAR sample store ${config.storeUrl}`);process.on('SIGTERM',async()=>{await app.close();await db.end();});
