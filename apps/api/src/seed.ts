...
export async function populate(tx:Tx) {
 ...
 const user=await tx.create('users',{business_key:'consumer',owner_id:'preset',data:{name:'青山 はるか',email:'haruka@example.test',preset:'consumer',language:'ja',timezone:'Asia/Tokyo',kyc:'verified',capabilities:caps,password_hash:passwordHash(randomToken()),sample:true}});
