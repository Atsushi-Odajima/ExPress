...
- ようこそ画面のログイン欄は**見せかけの表示**。入力は読み取り専用で認証に使わず、「デモアカウントで開始」は `/v1/demo/start` を呼んで隔離workspaceを開く（大きなボタンと同じ）。表示値は `DEMO_LOGIN_ID`／`DEMO_LOGIN_PASSWORD`（既定 `kuro`／`0130`）。同じ資格情報は API の `/v1/auth/login` でも実際に通る（`tests/demo-login.test.ts`）が、画面はそれに依存しない。
