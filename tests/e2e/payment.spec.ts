...
test('英語切替・加盟店検索・共有QR・運営者の能力別設定',async({page},testInfo)=>{
 ...
 await page.getByLabel('Switch demo persona').selectOption('merchant1_owner');await expect(page.getByLabel('Switch demo persona')).toHaveValue('merchant1_owner');await expect(page.getByRole('heading',{name:'Merchant overview',exact:true})).toBeVisible();
 ...
 await page.getByLabel('Switch demo persona').selectOption('admin');await expect(page.getByLabel('Switch demo persona')).toHaveValue('admin');await page.goto('/admin/accounts');await page.getByRole('button',{name:'Review / restrict',exact:true}).first().click();await expect(page.getByRole('checkbox',{name:'New payments',exact:true})).toBeChecked();await page.getByRole('button',{name:'Cancel',exact:true}).click();
...
test('WebhookでECが支払済みに収束、500再送・鍵更新・重複・順序逆転で二重発送なし',async({page,request})=>{
 ...
 // Scenario 1: the store server verifies the order and captures immediately; no return-URL parameter marks anything paid.
 const confirmed=await page.request.post(STORE+'/api/orders/'+order.id+'/confirm',{headers:{...sh,'idempotency-key':crypto.randomUUID()},data:{}});expect(confirmed.ok(),await confirmed.text()).toBeTruthy();const confirmedOrder=await confirmed.json();expect(confirmedOrder.status).toBe('paid');expect(confirmedOrder.shipments).toHaveLength(1);expect(confirmedOrder.payment.captured).toBe('12000');
 ...
 // The independent worker delivers after the business commit; wait for the injected 500 before retrying every delivery.
 await expect.poll(async()=>{const rows=await (await page.request.get(API+'/v1/webhook-deliveries?limit=100')).json();return rows.data.some((d:any)=>d.data.http_status===500);},{timeout:20000}).toBe(true);
 const deliveries=await (await page.request.get(API+'/v1/webhook-deliveries?limit=100')).json();
...
test('加盟店・運営者・PWA・ダークモード',async({page},testInfo)=>{
 ...
 await page.getByLabel('デモ人物を切り替え').selectOption('merchant1_owner');await expect(page.getByLabel('デモ人物を切り替え')).toHaveValue('merchant1_owner');await expect(page.getByRole('heading',{name:'加盟店ダッシュボード'})).toBeVisible();await page.screenshot({path:`test-results/${testInfo.project.name}-merchant.png`,fullPage:true});
 await page.getByLabel('デモ人物を切り替え').selectOption('merchant1_read_only');await expect(page.getByLabel('デモ人物を切り替え')).toHaveValue('merchant1_read_only');await expect(page.getByRole('heading',{name:'加盟店ダッシュボード'})).toBeVisible();await page.goto('/merchant/payouts');const forbidden=()=>page.getByRole('alert').filter({hasText:'権限'});await expect(forbidden()).toContainText('この画面を利用する権限がありません');await expect(forbidden()).toContainText('payouts:write');await expect(page.getByRole('button',{name:'出金を申請'})).toHaveCount(0);await page.goto('/developer');await expect(forbidden()).toContainText('webhooks:manage');
 await page.getByLabel('デモ人物を切り替え').selectOption('admin');await expect(page.getByLabel('デモ人物を切り替え')).toHaveValue('admin');await expect(page.getByRole('heading',{name:'運営ダッシュボード'})).toBeVisible();await expect(page.getByText('検査OK')).toBeVisible();
...
test('Playgroundで実注文作成・APIログ・管理者の調整仕訳と反対仕訳',async({page})=>{
 ...
 await page.getByLabel('デモ人物を切り替え').selectOption('consumer');await expect(page.getByLabel('デモ人物を切り替え')).toHaveValue('consumer');await expect(page.locator('.hero-balance')).toContainText('30,000');
});
...
test('デモパネルの同時承認で過剰使用を防ぎ、100重複captureが1回になる',async({page})=>{
 ...
 await page.getByLabel('デモ人物を切り替え').selectOption('merchant1_owner');await expect(page.getByLabel('デモ人物を切り替え')).toHaveValue('merchant1_owner');await expect(page.getByRole('heading',{name:'加盟店ダッシュボード'})).toBeVisible();await page.goto('/demo');await expect(page.getByRole('heading',{name:'100並列の売上確定を試す'})).toBeVisible();await page.getByLabel('オーソリ',{exact:true}).selectOption({index:1});await page.getByRole('button',{name:'内容を確認',exact:true}).click();await page.getByRole('button',{name:'確認して実行',exact:true}).click();await expect(page.getByLabel('並列要求の結果')).toContainText('"unique_captures": 1');await expect(page.getByLabel('並列要求の結果')).toContainText('"ledger_ok": true');
});
