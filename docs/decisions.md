...
- ようこそ画面のログイン欄は見せかけに切り替えた。無料インスタンスの起動待ちで実ログインが失敗すると画面から進めなくなるため、表示は読み取り専用にし、「デモアカウントで開始」は大きなボタンと同じ `/v1/demo/start` を呼ぶ。API の `/v1/auth/login` と組み込み資格情報は実際に機能する状態のまま残す（見せかけの表示が嘘にならないように）。あわせて、読み込み時に `/v1/health` で API を起こし、開始操作は起動待ちの間だけ自動で再試行する。
- 「デモ・実際のお金は動きません」の表示はTOP画面のヘッダー1か所だけにした（起動画面・サイドバー・topbarのバッジ・ウォレットの2箇所・チェックアウト・フッター・明細/確認ダイアログから削除）。ポートフォリオとして全ページに出るとくどいという指摘による。ロゴとアプリ名は全画面の左上に出す：広い画面はサイドバー、700px以下はtopbarの `.topbar-brand`（同時に表示しないよう幅で切り替え、狭い画面ではパンくずの見出しを隠す）。サンプル利用者は `demo josep` に改名。
- 開始処理の待機に `loading` を流用したため、無料インスタンスの復帰待ち（実測35秒）の間ずっと起動スプラッシュ（`loading&&!session`）が出て、操作不能に見えていた。待機用に `starting` を分け、ようこそ画面を出したままボタンに経過秒数を出す方式へ変更。再試行は3秒間隔・最大3分。初回読み込みで API が復帰待ちのときはエラーを出さず、ようこそ画面をそのまま見せる。
