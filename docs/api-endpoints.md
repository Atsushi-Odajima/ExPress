...
|PATCH|`/v1/merchant/profile`|店舗プロフィール・出金先設定 Allowed roles: owner. Session mutations require exact Origin and X-CSRF-Token.|
|PATCH|`/v1/merchant/return-urls`|戻り先URL登録 Allowed roles: owner, developer. Session mutations require exact Origin and X-CSRF-Token.|
|POST|`/v1/merchant/applications`|加盟店アプリの登録 Session mutations require exact Origin and X-CSRF-Token.|
|POST|`/v1/merchant/applications/{id}/rotate`|API秘密鍵 rotate Allowed roles: owner, developer. Session mutations require exact Origin and X-CSRF-Token.|
|POST|`/v1/merchant/applications/{id}/revoke`|API秘密鍵 revoke Allowed roles: owner, developer. Session mutations require exact Origin and X-CSRF-Token.|
...
|POST|`/v1/payment-links/{id}/deactivate`|支払リンク無効化 Required scope: orders:write. Session mutations require exact Origin and X-CSRF-Token.|
|POST|`/v1/checkout/payment-links/{id}/open`|支払リンクの確認画面を開く Session mutations require exact Origin and X-CSRF-Token.|
|POST|`/v1/webhook-endpoints`|Webhook通知先登録 Required scope: webhooks:manage. Session mutations require exact Origin and X-CSRF-Token.|
...
|PATCH|`/v1/webhook-endpoints/{id}`|Webhook通知先更新 Required scope: webhooks:manage. Session mutations require exact Origin and X-CSRF-Token.|
|DELETE|`/v1/webhook-endpoints/{id}`|Webhook通知停止 Required scope: webhooks:manage. Session mutations require exact Origin and X-CSRF-Token.|
|POST|`/v1/webhook-endpoints/{id}/rotate-secret`|署名鍵更新・旧鍵は24時間移行 Required scope: webhooks:manage. Session mutations require exact Origin and X-CSRF-Token.|
|POST|`/v1/webhook-endpoints/{id}/test`|テスト通知 Required scope: webhooks:manage. Session mutations require exact Origin and X-CSRF-Token.|
|POST|`/v1/webhook-deliveries/{id}/retry`|同一イベントを再配信 Required scope: webhooks:manage. Session mutations require exact Origin and X-CSRF-Token.|
...
|POST|`/v1/subscriptions/{id}/pause`|購読 pause Required scope: subscriptions:write. Session mutations require exact Origin and X-CSRF-Token.|
|POST|`/v1/subscriptions/{id}/resume`|購読 resume Required scope: subscriptions:write. Session mutations require exact Origin and X-CSRF-Token.|
|POST|`/v1/subscriptions/{id}/cancel`|購読 cancel Required scope: subscriptions:write. Session mutations require exact Origin and X-CSRF-Token.|
|POST|`/v1/disputes/{id}/messages`|購入案件に回答 Required scope: payments:read. Allowed roles: owner, support, finance, api. Session mutations require exact Origin and X-CSRF-Token.|
